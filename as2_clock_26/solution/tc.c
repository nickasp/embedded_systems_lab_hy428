...
static inline uint32_t tc_periph_id(Tc *tc, uint8_t chan)
{
    ...
    if(tc == TC1 && chan == 3) return ID_TC3;
    if(tc == TC1 && chan == 4) return ID_TC4;
    if(tc == TC1 && chan == 5) return ID_TC5;
    if(tc == TC2 && chan == 6) return ID_TC6;
    if(tc == TC2 && chan == 7) return ID_TC7;
    if(tc == TC2 && chan == 8) return ID_TC8;
 ...
 * channel 'chan' (channel 0,1 or 2 of timer counter 'tc') (CLOCK1 = MCK/2 = 42 MHz),
...
void tc_init(Tc *tc, uint8_t chan, uint32_t rc)
{
	...
	if(id == 0) return;
	//enables peripheral clock for peripheral with ID 'id' (R0 is responsible for 0-31 )
       	PMC->PMC_PCER0 = (1u << id);
...
void tc_pwm_init(Tc *tc, uint8_t ch, uint32_t T, uint8_t D)
{
	...
	//checks for invalid parameters 
	//if(!tc || ch > 2u) return;
	if(!tc || ch > 2u || D == 0 || T == 0 || D >= T) return;	

	//
	//(1) enables peripheral clock in PMC (required) 
        //
	id = tc_periph_id(tc, ch);
    	//checks if invalid id
	if(id == 0) return;
	...
	//(2) disables 'chan' channel clock for further configuration
	//
	tc->TC_CHANNEL[ch].TC_CCR = TC_CCR_CLKDIS;
	
	//
	//(3) disables all interrupts for this channel (writes 11111111 to interrupt-disable register)
	//
	tc->TC_CHANNEL[ch].TC_IDR = 0xFF;
	...
	//
	//(5) sets RC to rc value and RA to ra value
	//
	tc->TC_CHANNEL[ch].TC_RC = T;
	tc->TC_CHANNEL[ch].TC_RA = D;
...
void tc_spinus(Tc *tc, uint8_t channel, uint32_t us)
{
	...
	//disables clock for proceeding into configuring RC
	//tc->TC_CHANNEL[chan].TC_CCR = TC_CCR_CLKDIS;
...
void tc_int_disable(Tc *tc, uint8_t ch, uint32_t rc)
{
	...
	//disables clock for proceeding into further configuration
	//tc->TC_CHANNEL[ch].TC_CCR = TC_CCR_CLKDIS;
