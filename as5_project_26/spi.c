...
void spi_init(void){
	...
	// enables PIOC to control pins 21 (spi cd), 22 (spi rst) 
	PIOC->PIO_PER = (1u << 21) | (1u << 22);

	// enables PIOD to control pin 7 (spi mosi)
	PIOD->PIO_PER = (1u << 7);

	
	// configures CS (Chip Select) as output pin
	PIOA->PIO_OER = (1u << 28);

	// configures SCK (Serial Clock) as output pin
	PIOB->PIO_OER = (1u << 27);

	// configures CD (command/data) and RST (reset) as output pins
	PIOC->PIO_OER = (1u << 21) | (1u << 22);

	// configures MOSI (Master Output, Slave Input) as output pin
	PIOD->PIO_OER = (1u << 7);


	// Set CS as HIGH (display disabled)
	PIOA->PIO_SODR = (1u << 28);
...
void spi_writebit(uint8_t byte){
	...
	// waits for setup time (~1 microsecond for SPI timing margin)
	tc_spinus(TC0, 1, 1);

	// enables clock (high)
	PIOB->PIO_SODR = (1u << 27);

	// waits for another ~1 microsecond
	tc_spinus(TC0, 1, 1);
