...
void tc_init(Tc *tc, uint8_t chan, uint32_t rc)
{
	...
	if(id == 0) return;
	//enables peripheral with ID 'id' value 
	//(R0 is responsible for 0-31 and R1 is responsible for 32,33 ... and so on ...)
    	 if(id>=27 && id<=31)
		PMC->PMC_PCER0 |= (1u << id);
	else
		PMC->PMC_PCER1 |= (1u << (id-32u));
...
void tc_pwm_init(Tc *tc, uint8_t c, uint32_t T, uint32_t D)
{
	uint32_t id;

	//checks for invalid parameters 
	if(!tc || c > 2u || D == 0 || D >= T) return;	
	...
	if(id == 0) return;
	//enables peripheral with ID 'id' value 
	//(R0 is responsible for 0-31 and R1 is responsible for 32-...)
       	if(id>=27 && id<=31)
		PMC->PMC_PCER0 |= (1u << id);
	else
		PMC->PMC_PCER1 |= (1u << (id-32u));
...
void tc_spinus(Tc *tc, uint8_t channel, uint32_t us)
{
    ...
    ticks = (((uint64_t)SystemCoreClock / (uint64_t)div)/1000000ULL * (uint64_t)us);
